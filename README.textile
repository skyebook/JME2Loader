...
** _I built a model, converted it to the jME2 file format, and {select excuse: lost || deleted || didn't save the source files_
* User-contributed assets in an online system
** _Users contributed their time an effort to build assets, and only the .jme files were saved on the server. oops._
* Workaround for getting model formats that were supported in jME2
